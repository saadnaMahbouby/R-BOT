...
export const ButtonsBlockSettings = ({ options, onOptionsChange }: Props) => {
  ...
  const updateWhatsappListBodyText = (whatsappListBodyText: string) =>
    onOptionsChange({ ...options, whatsappListBodyText });
  const updateWhatsappListButtonText = (whatsappListButtonText: string) =>
    onOptionsChange({ ...options, whatsappListButtonText });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Nouveaux champs pour WhatsApp List - CORRIGÉ */}
      <Field.Container>
        <div className="font-semibold mb-2">
          WhatsApp List Settings (4+ choices)
        </div>
        <Field.Root>
          <Field.Label>
            List Body Text
            <MoreInfoTooltip>
              Text shown in the WhatsApp list message body when you have more than 3 choices
            </MoreInfoTooltip>
          </Field.Label>
          <DebouncedTextInputWithVariablesButton
            defaultValue={options?.whatsappListBodyText ?? "Choisissez une option"}
            onValueChange={updateWhatsappListBodyText}
          />
        </Field.Root>
        <Field.Root>
          <Field.Label>
            List Button Text
            <MoreInfoTooltip>
              Text shown on the button that opens the WhatsApp list
            </MoreInfoTooltip>
          </Field.Label>
          <DebouncedTextInputWithVariablesButton
            defaultValue={options?.whatsappListButtonText ?? "Voir les options"}
            onValueChange={updateWhatsappListButtonText}
          />
        </Field.Root>
      </Field.Container>
