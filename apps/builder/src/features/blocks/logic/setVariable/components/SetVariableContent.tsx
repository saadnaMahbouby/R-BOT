...
const Expression = ({
  ...
  switch (options?.type) {
    ...
    case "Map item with same index": {
      ...
      return (
        <span>
          {variableName}= item in ${targetListVariable?.name} with same index as ${baseItemVariable?.name} in ${baseListVariable?.name}
        </span>
      );
    }
    case "Append value(s)": {
    ...
    case "Meta Body": {
      return (
        <span>
          {variableName}={" "}
          <Badge colorScheme="blue">Meta webhook body data</Badge>
        </span>
      );
    }
    default:
