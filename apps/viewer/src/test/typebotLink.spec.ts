...
test("should work as expected", async ({ page }) => {
  ...
  await page.goto(
    `${env.NEXTAUTH_URL}/bots/${publicTypebot1.typebotId}/results`,
  );
...
test.describe("Merge disabled", () => {
  test("should work as expected", async ({ page }) => {
    ...
    await expect(page.getByText("Cheers!")).toBeVisible();
    await page.goto(
      `${process.env.NEXTAUTH_URL}/bots/${publicTypebot1MergeDisabled.typebotId}/results`,
    );
    await expect(page.locator("text=Submitted at")).toBeVisible();
    await expect(page.locator("text=Hello there!")).toBeHidden();
    await page.goto(
      `${env.NEXTAUTH_URL}/bots/${publicTypebot2.typebotId}/results`,
    );
    await expect(page.locator("text=Hello there!")).toBeVisible();
  });
});
