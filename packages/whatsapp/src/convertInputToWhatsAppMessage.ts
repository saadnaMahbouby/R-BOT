...
export const convertInputToWhatsAppMessages = async ({
  ...
  switch (input.type) {
    ...
    case InputBlockType.CHOICE: {
      ...
      const filteredItems = input.items.filter((item) => isDefined(item.content));
      
      // Si plus de 3 choix, utiliser une liste WhatsApp
      if (filteredItems.length > 3) {
        return [{
          ...
          interactive: {
            type: "list",
            body: {
              text: lastMessageText || input.options?.whatsappListBodyText || "Choisissez une option",
            },
            action: {
              button: input.options?.whatsappListButtonText || "Voir les options",
              sections: [{
                title: "Options",
                rows: filteredItems.slice(0, 10).map((item) => ({
                  id: item.id,
      ...
      // Si 3 choix ou moins, utiliser des boutons
